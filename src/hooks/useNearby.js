...
function getCategory(tags) {
  ...
  if (tourism === 'attraction')          return { label: '景點',   icon: '📍' }
  if (historic)                          return { label: '古蹟',   icon: '🏯' }
  if (leisure === 'nature_reserve')      return { label: '自然區', icon: '🌿' }
  if (leisure === 'park' || leisure === 'garden') return { label: '公園', icon: '🌳' }
...
export function useNearby(stationId, lat, lng, enabled) {
  ...
  if (enabled && fetchedRef.current !== stationId) {
    fetchedRef.current = stationId
    if (cache.current[stationId]) {
      setItems(cache.current[stationId])
    } else {
      setLoading(true)
      setItems(null)
      setError(false)
      fetch(OVERPASS_URL, {
        ...
        .then(data => {
          const results = data.elements
            ...
            .slice(0, 12)
          cache.current[stationId] = results
          setItems(results)
        })
        .catch(() => setError(true))
